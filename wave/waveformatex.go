...
func (guid *GUID) Size() int {
	return 16
}

func (guid *GUID) ReadFrom(r io.Reader) (n int64, err error) {
	if err = binary.Read(r, binary.LittleEndian, &guid.Data1); err != nil {
		return
	}
	n += 4

	if err = binary.Read(r, binary.LittleEndian, &guid.Data2); err != nil {
		return
	}
	n += 2

	if err = binary.Read(r, binary.LittleEndian, &guid.Data3); err != nil {
		return
	}
	n += 2

	var rd int
	rd, err = r.Read(guid.Data4[:])
	n += int64(rd)
	return
}

func (guid *GUID) WriteTo(w io.Writer) (n int64, err error) {
...
func (wfex *WaveFormatEx) Size() int {
	return 16
}

func (wfex *WaveFormatEx) ReadFrom(r io.Reader) (n int64, err error) {
	if err = binary.Read(r, binary.LittleEndian, &wfex.FormatTag); err != nil {
		return
	}
	n += 2

	if err = binary.Read(r, binary.LittleEndian, &wfex.Channels); err != nil {
		return
	}
	n += 2

	if err = binary.Read(r, binary.LittleEndian, &wfex.SamplesPerSec); err != nil {
		return
	}
	n += 4

	if err = binary.Read(r, binary.LittleEndian, &wfex.AvgBytesPerSec); err != nil {
		return
	}
	n += 4

	if err = binary.Read(r, binary.LittleEndian, &wfex.BlockAlign); err != nil {
		return
	}
	n += 2

	if err = binary.Read(r, binary.LittleEndian, &wfex.BitsPerSample); err != nil {
		return
	}
	n += 2
	return
...
func (wfex *WaveFormatEx) WriteTo(w io.Writer) (n int64, err error) {
	if err = binary.Write(w, binary.LittleEndian, wfex.FormatTag); err != nil {
		return
	}
	n += 2

	if err = binary.Write(w, binary.LittleEndian, wfex.Channels); err != nil {
		return
	}
	n += 2

	if err = binary.Write(w, binary.LittleEndian, wfex.SamplesPerSec); err != nil {
		return
	}
	n += 4

	if err = binary.Write(w, binary.LittleEndian, wfex.AvgBytesPerSec); err != nil {
		return
	}
	n += 4

	if err = binary.Write(w, binary.LittleEndian, wfex.BlockAlign); err != nil {
		return
	}
	n += 2

	if err = binary.Write(w, binary.LittleEndian, wfex.BitsPerSample); err != nil {
		return
	}
	n += 2
	return
...
func (wfex *WaveFormatEx) Converter() (converter.Converter, error) {
	...
	case WAVE_FORMAT_PCM:
		...
		case 8:
			conv = converter.Uint8
		case 16:
			conv = converter.Int16
		case 24:
			conv = converter.Int24
		case 32:
			conv = converter.Int32
		}
	case WAVE_FORMAT_IEEE_FLOAT:
		switch wfex.BitsPerSample {
		case 32:
			conv = converter.Float32
		case 64:
			conv = converter.Float64
		}
	}
	if conv == nil {
		return nil, errors.New("wave: unsupported wave file format")
	}
	return conv, nil
}
...
func (wfext *WaveFormatExtensible) Size() int {
	return wfext.Format.Size() + int(wfext.Format.ExtSize)
}

func (wfext *WaveFormatExtensible) ReadFrom(r io.Reader) (n int64, err error) {
	if n, err = wfext.Format.ReadFrom(r); err != nil {
		return
	}

	if wfext.Format.FormatTag != WAVE_FORMAT_EXTENSIBLE {
		return
	}

	if err = binary.Read(r, binary.LittleEndian, &wfext.Format.ExtSize); err != nil {
		return
	}
	n += 2

	if wfext.Format.FormatTag == WAVE_FORMAT_EXTENSIBLE && wfext.Format.ExtSize != 22 {
		return n, errors.New("wave: unsupported wave file format")
	}
	...
	if err = binary.Read(r, binary.LittleEndian, &wfext.Samples); err != nil {
		return
	}
	n += 2

	if err = binary.Read(r, binary.LittleEndian, &wfext.ChannelMask); err != nil {
		return
	}
	n += 4

	var rd int64
	rd, err = wfext.SubFormat.ReadFrom(r)
	n += rd
	return
}

func (wfext *WaveFormatExtensible) WriteTo(w io.Writer) (n int64, err error) {
	if (wfext.Format.FormatTag != WAVE_FORMAT_EXTENSIBLE && wfext.Format.ExtSize != 0) ||
		(wfext.Format.FormatTag == WAVE_FORMAT_EXTENSIBLE && wfext.Format.ExtSize != 22) {
		return 0, errors.New("wave: unsupported wave file format")
	}

	if n, err = wfext.Format.WriteTo(w); err != nil {
		return
	}
	...
	if err = binary.Write(w, binary.LittleEndian, wfext.Format.ExtSize); err != nil {
		return
	}
	n += 2

	if err = binary.Write(w, binary.LittleEndian, wfext.Samples); err != nil {
		return
	}
	n += 2

	if err = binary.Write(w, binary.LittleEndian, wfext.ChannelMask); err != nil {
		return
	}
	n += 4

	var wt int64
	wt, err = wfext.SubFormat.WriteTo(w)
	n += wt
	return
}
